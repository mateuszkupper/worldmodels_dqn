...
class DQN_Agent:
    ...
    def initialize_tf_variables(self):
        ...
        # Tf placeholders
        self.state_tf = tf.placeholder(shape=[None, 32, 10, 1], dtype=tf.float32, name='state_tf')
    ...
    def experience_replay(self, alpha):
        ...
        fixed_feed_dict = {self.state_tf: np.reshape(next_state_batch, (32, 32, 10, 1))}
        ...
        feed = {self.state_tf: np.reshape(state_batch, (32, 32, 10, 1)), self.action_tf: action_batch, self.y_tf: y_batch, self.alpha: alpha}
    ...
    def get_action(self, state, epsilon):
        ...
        if random.random() < epsilon:
            ...
            return self.sess.run(self.Q_argmax, feed_dict={self.state_tf: np.reshape(state, (1, 32, 10, 1))})[0]
    ...
    def train(self):
        while self.sess.run(self.episode) < self.training_metadata.num_episodes:
            ...
            if (episode % 20 == 0) and (episode != 0):
                ...
                with open("CarRacing.txt", append_write) as f:
                    f.write(str(score) + "," + str(std) + "\n")
